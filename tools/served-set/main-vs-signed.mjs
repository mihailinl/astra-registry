...
//
//   * a SERIAL difference is measured from the newest commit on main's
//     FIRST-PARENT line that changed what main holds under
//     `tools/revocations/` — the moment the change became reachable from
//     `main`. The serial is `rev-list --count <sha> -- SERIAL_PATHSPEC` + 1,
//     a count of what is reachable, so it moves at that moment: for a direct
//     push or a squash that is the commit itself, and for a pull request
//     merged with a merge commit it is the MERGE. Only a commit that changes
//     main's copy of the directory can move the window. The clock reads the
//     same export as the signer's `serialsAt` rather than a copy of the
//     string, because a copy is what let the two be widened apart (gap 64);
//
//     `--first-parent` is the whole of that sentence (gap 68). A plain
//     `git log -1 -- <pathspec>` simplifies history through a merge, which is
//     TREESAME to its branch parent for the path, and dates the BRANCH commit:

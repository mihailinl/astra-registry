...
export const PHASE_3_CHECKS = [
  ...
  },
  {
    ...
    why: "the DELTA half landed in bot/lib/policy.mjs (task 3.5), on the release path only: a release asking for authority the newest listed version did not have is held (R_NEW_HIGH_RISK) or delayed (P_DELAY_WIDENED). The ENFORCEMENT half has since landed too — require_permission runs at the top of every host RPC that needs one, HOST_RPC_PERMISSIONS in astra-daemon/src/plugins/host_service.rs is the table and a test reads it rather than the methods, and astra-ui's InstallConsentSheet shows the set before an install — so a plugin that declares nothing and calls SetThemeContribution or SendChatMessage anyway IS refused on the user's machine. What is still implemented nowhere is the CEILING: nothing caps what a tier may request, so tier 3 (a sideloaded directory) can take every permission in the vocabulary by editing its own manifest. And none of it runs on THIS path: a hand-edited listing is diffed by its reviewer.",
  },
  {
    code: "E_REVOKED",

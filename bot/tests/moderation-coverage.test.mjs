...
// the names with no credential, and from contract 1.0.0 (ops pending item 28)
// TRUST-44 reads them too, so they are pins it compares. `policy/settings-expected.json`
...
// hour for its read of the settings ROLL-7 pins, and gives that read as exact
// calls: `/rulesets`, one `/rulesets/{id}` per ruleset, `/rules/branches/main`,
// `/environments`, one `/environments/{name}/deployment-branch-policies` per
// pinned environment, and `/branches/main` — 4 + R + E, 10 today, 11 once
// `bot-state` is pinned at R5 (ID-12 gives the lookups 34 and keeps 14
// unspent). Contract 0.36.0 read without the policy names, 4 + R within 6;
// 1.0.0 (ops pending item 28, the contract's first MAJOR) reads them, and moved
// the reservation with them. The 12 lives in the contract and the rulesets and
// environments live in `policy/settings-expected.json`, and an owner who
// creates an environment changes the second without anyone opening the first:
// the service's hourly read would then overrun its share, and its mint lookups
// would start answering `rate_limited` for a reason nothing records. So the two
// are compared here. Environments pending creation count, because each is
// created to be pinned (RC-R0-3(f)).
...
function trust44Overrun(expected) {
  ...
  return `TRUST-44's read of astra-registry is 4 + R + E = ${calls} calls for the rulesets and environments ` +
    "policy/settings-expected.json holds (pending ones counted, since each is created to be pinned), and the " +
    `contract reserves ${TRUST44_RESERVATION} for it (TRUST-44; ID-12; since contract 1.0.0). A pin past the ` +
    "reservation waits for a contract version that raises it, published before the environment or ruleset is created";
}

test("repo-settings: TRUST-44's read of what ROLL-7 pins fits the 12 calls the contract reserves for it", () => {
  const doc = settingsDoc();
  const expected = doc.repositories[checkoutSlug(doc)];
  const calls = trust44Calls(expected);
  ...
  // Proven on the committed expectation grown up to the reservation, which
  // fits, and one past it, which does not — by environments, live and
  // pending, and by rulesets: an owner's next settings acts. Grown relative to
  // today's count, so an owner who spends the room left fails only the fit.
  const grown = (n, group) => {
    const g = structuredClone(expected);
    const model = structuredClone(Object.values(expected[group === "rulesets" ? "rulesets" : "environments"])[0]);
    g[group] = { ...(g[group] ?? {}) };
    for (let i = 0; i < n; i++) g[group][`extra-${i}`] = structuredClone(model);
    assert.equal(trust44Calls(g), calls + n, `growing ${group} by ${n} did not move the count`);
    return g;
  };
  const room = TRUST44_RESERVATION - calls;
  for (const group of ["environments", "pending_environments", "rulesets"]) {
    assert.equal(trust44Overrun(grown(room, group)), null, `${group} grown to exactly ${TRUST44_RESERVATION} calls still fit, and were refused`);
    const over = trust44Overrun(grown(room + 1, group));
    assert.ok(over && over.includes(`= ${TRUST44_RESERVATION + 1} calls`) && over.includes(`reserves ${TRUST44_RESERVATION}`) &&
      over.includes("contract 1.0.0"),
    `${group} grown to ${TRUST44_RESERVATION + 1} calls and the check said ${JSON.stringify(over)}`);
  }
